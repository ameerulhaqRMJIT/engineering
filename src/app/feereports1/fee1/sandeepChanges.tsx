...
const FeePaymentForm: React.FC = () => {
  ...
  const [feeDetails, setFeeDetails] =  useState<Student[]>([]); // State to hold fee details
  ...
  const handleUpdate = async () => {
    try {
      ...
      // } else {
      //   setAlertMessage(response?.message || 'Failed to update transport details.');
  ...
  const handleCheckboxChange = (index: number) => (event: React.ChangeEvent<HTMLInputElement>) => {
    setSelectedRowIndex(event.target.checked ? index : null);
  };
              ...
              <TableBody>
                {feeDetails.map((fee, index) => (
                  <TableRow key={index}>
                    ...
                    {/* <TableCell>{fee.year}</TableCell>onChange={handleCheckboxChange} 
                    <TableCell>{fee.feeName}</TableCell>l>
                    <TableCell>{fee.paid}</TableCe
                    <TableCell>{fee.amount}</TableCelll>
