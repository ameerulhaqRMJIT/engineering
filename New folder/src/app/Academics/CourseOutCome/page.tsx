...
const Lessonplan: React.FC = () => {
    ...
    const [outcome1, setdobjectives] = useState<string>('');
    useEffect(() => {
        fetchClassView();
      }, []);
    ...
    const handleCloseSnackbar = () => {
      ...
      const fetchlessonView = async () => {
        try {
          ...
          const token = localStorage.getItem('token') || undefined;
          const username = localStorage.getItem('username');
          const Url = `${DIGITAL_CAMPUS_BASE_URL}/CourseoutcomeByParams?subjectcode=${subjectcode}&facultyid=${username}&branchid=${branchid}&acadamicid=${acadamicid}&year=${year}&sem=${sem}&regulation=${regulation}&batch=${batch}`;
      ...
      const populateFormFields = (data: SelectedData) => {
        setNewClass({
            ...
            cosname:data.cosname,
            bloomstaxonomy:data.bloomstaxonomy,
      ...
      const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
          ...
          const requestData = {
            ...
            cosname:newClass.cosname,
            bloomstaxonomy:newClass.bloomstaxonomy,
            deletestatus:newClass.deletestatus,
          };

          const apiEndpoint = `${DIGITAL_CAMPUS_BASE_URL}/CourseOutComeSave`;
          const apiEndpointclassupdate = `${DIGITAL_CAMPUS_BASE_URL}/CourseOutComeUpdate`;
          if (newClass.id) {
  ...
  return (
    <>
    <Layout>
  <PageTitle title='Course Out Come'/>
      <CustomSnackbar
        open={isSnackbarOpen}
...
<TableBody>
   {classes.map((classItem,index) => (
     <TableRow key={index}>
        <TableCell align="center">{classItem.outcome}</TableCell>
            <TableCell>
